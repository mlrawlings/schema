...
Schema.prototype.validateField = function(object, key, strict_mode, definition) {
	...
	if(!definition)
		if(this.options.dynamic) return { result:true }
		else return { result:false, errors:[new Error(key + ' is not in the schema.')] }	
	...
	try {
		...
		return { result:false, errors:[new Error('Expected ' + definition.name + ' to be a ' + definition.type.name + '.')] }
	}
